...
# Set the CUDA devices that should be visible to the script
...
for step in "${timesteps[@]}"; do
    ...
    FID_DIR="${SAVE_DIR}/fid" # New FID directory within each timestep directory
    FID_OUTPUT_FILE="${FID_DIR}/fid_results.txt" # FID output file within the new FID directory

...
class Diffusion(object):
    ...
    def sample_image(self, x, model, last=True, classifier=None, base_samples=None):
        ...
        if self.args.sample_type == "generalized" or self.args.sample_type == "generalized_accel":
            ...
            from functions.denoising import generalized_steps, generalized_steps_accelerated
            ...
            elif self.args.sample_type == "generalized_accel":
                xs, _ = generalized_steps_accelerated(x, seq, model_fn, self.betas, eta=self.args.eta, classifier=classifier, is_cond_classifier=self.config.sampling.cond_class, classifier_scale=classifier_scale, **model_kwargs)
            x = xs[-1]
        elif self.args.sample_type == "ddpm_noisy" or self.args.sample_type == "ddpm_noisy_accel":
            if self.args.skip_type == "uniform":
                skip = self.num_timesteps // self.args.timesteps
                seq = range(0, self.num_timesteps, skip)
            elif self.args.skip_type == "quad":
                ...
                raise NotImplementedError
            from functions.denoising import ddpm_steps,ddpm_steps_accelerated
            def model_fn(x, t, **model_kwargs):
                out = model(x, t, **model_kwargs)
                if "out_channels" in self.config.model.__dict__.keys():
                    if self.config.model.out_channels == 6:
            ...
            elif self.args.sample_type == "ddpm_noisy_accel":
                xs, _ = ddpm_steps_accelerated(x, seq, model_fn, self.betas, classifier=classifier, is_cond_classifier=self.config.sampling.cond_class, classifier_scale=classifier_scale, **model_kwargs)
            x = xs[-1]
